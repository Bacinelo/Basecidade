...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookjoins = "https://discord.com/api/webhooks/842898294808379393/TycphxCiH_H_EjYijGu9dsadsadsasaCRMCUj7k4EjltWUl8VzHC_q4mdaND5u5Vf9kmO5izXYBTmge"
local webhookexit = "https://discord.com/api/webhooks/842898353963663412/PnDE8kGsjO6m8KqWVCLskAldsadsadsayPXeArckgRLapb7bs2MaLuJHO8nZmfG6uJxr1LbcSOyJ5"
...
function task_save_datatables()
	SetTimeout(300000,task_save_datatables)
	TriggerEvent("vRP:save")
	for k,v in pairs(vRP.user_tables) do
		vRP.setUData(k,"Datatable",json.encode(v))
	end
end
	...
	if steam then
		if not vRP.isBanned(steam) then
			if vRP.isWhitelisted(steam) then
				...
				deferrals.done("Faca sua whitelist e na parte de steam hex coloque apenas os números: "..steam)
...
AddEventHandler("baseModule:idLoaded",function(source,user_id,model)
	...
	if vRP.rusers[user_id] == nil then
		...
		if model ~= nil then
			...
			vRP.user_tables[user_id].inventorys["2"] = { item = "sandwich", amount = 2 }
			vRP.user_tables[user_id].inventorys["3"] = { item = "water", amount = 2 }
			vRP.user_tables[user_id].inventorys["4"] = { item = "dollars", amount = 8000 }
		end
